...
@router.get("/listas", response_model=list[schemas.ListaResumo]) #lista geral com tudo
def listar_listas(db: Session = Depends(get_db),
                  user_id: int = Depends(get_current_user)):
    return db.query(Lista).filter(Lista.user_id == user_id).order_by(Lista.created_at.desc()).all()


@router.get("/listas/{lista_id}", response_model=schemas.ListaDetalhe) #tudo que esta dentro da lista
def detalhe_lista(lista_id: int, db: Session = Depends(get_db),
                  user_id: int = Depends(get_current_user)):
    lista = db.query(Lista).filter(Lista.id == lista_id, Lista.user_id == user_id).first()
    if lista is None:
...
def criar_lista(dados: schemas.ListaCriar, 
